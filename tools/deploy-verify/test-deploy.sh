...
#       ENV_ID 也可写在 <target>/scripts/.env 中（参考 .env.example），脚本会自动加载。
#
# 此脚本由各项目的 scripts/test-deploy.sh wrapper 调用，wrapper 会预先设置 CF_ROOT。
# Usage:
#   通过项目 wrapper 调用：
#     cloudfunctions/scripts/test-deploy.sh [options]
#     cloudbaserun/scripts/test-deploy.sh [options]
...
Options:
  ...
  --clean-up      只执行清理（来源 = 本仓库子目录 + .env::PREFIX），不部署不调用
                  Only run cleanup (sources = local subdirs + .env::PREFIX); skip deploy/invoke
  --dry-run       仅打印命令，不执行 / Print commands without executing
  -h, --help      显示帮助
Environment:
  ENV_ID          CloudBase 环境 ID（必填，也可写在 <target>/scripts/.env）
                  CloudBase env id (required; may also be set in <target>/scripts/.env)
EOF
}
...
if [ -z "${ENV_ID:-}" ]; then
    log_fail "请先 export ENV_ID 或在 $CF_ROOT/scripts/.env 中设置 ENV_ID"
    log_fail "Please export ENV_ID or set ENV_ID in $CF_ROOT/scripts/.env (see .env.example)"
    exit 2
fi
...
# --clean-up 模式：扫描目标目录下 */cloudbaserc.json 拿到函数列表（目录名），
# 结合 .env 中的 ENV_ID 与 PREFIX 计算 deploy_name=PREFIX+目录名，先删 service 再删 function。
# --clean-up mode: enumerate functions by scanning */cloudbaserc.json
...
run_cleanup_only() {
    ...
    if [ -z "${PREFIX:-}" ]; then
        ...
        log_warn "  如需删除测试函数，请使用：--prefix test-  或在 $CF_ROOT/scripts/.env 设置 TEST_NAME_PREFIX=test-"
        log_warn "  To target test functions, pass --prefix test- or set TEST_NAME_PREFIX=test- in $CF_ROOT/scripts/.env"
    fi

    # 扫描当前目标目录下含 cloudbaserc.json 的子目录，目录名即函数名
    # Enumerate function names by scanning */cloudbaserc.json
    local local_names
    ...
    log_info "扫描到 $total 个本地目录 / Found $total local director(ies)"
...
log_step "Tier 2 部署 + 调用 / Deploy + invoke against ENV_ID=$ENV_ID (target: $CF_ROOT)"
while IFS= read -r name; do
    [ -z "$name" ] && continue
    deploy_one "$name"
done <<< "$names"
