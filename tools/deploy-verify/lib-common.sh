#!/bin/bash
# 通用工具 / Shared helpers for deploy verification scripts
# 用法：在脚本顶部 `source "$SCRIPT_DIR/lib-common.sh"`
# CF_ROOT 应由调用方（wrapper）预先 export，指向 cloudfunctions/ 或 cloudbaserun/。
# 若未设置，则自动推导为脚本所在目录的上级目录（兼容直接调用）。
...
# ---- 路径 / Path helpers -------------------------------------------------
# SCRIPT_DIR = 本文件所在目录 (tools/deploy-verify/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# CF_ROOT 由 wrapper 预先 export 设置；如果未设置则自动推导（兼容旧行为）
# CF_ROOT should be pre-set by wrapper scripts; auto-detect otherwise (backward compat)
if [ -z "${CF_ROOT:-}" ]; then
    CF_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
fi

MATRIX_FILE="$CF_ROOT/scripts/test-matrix.json"
REPORT_FILE="$CF_ROOT/scripts/.last-report.json"
ENV_FILE="$CF_ROOT/scripts/.env"
