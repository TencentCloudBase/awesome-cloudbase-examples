...
#
# 此脚本由各项目的 scripts/test-local.sh wrapper 调用，wrapper 会预先设置 CF_ROOT。
# Usage:
#   通过项目 wrapper 调用：
#     cloudfunctions/scripts/test-local.sh [--tier=0|1|all] [--only a,b] [--skip c,d]
#     cloudbaserun/scripts/test-local.sh [--tier=0|1|all] [--only a,b] [--skip c,d]
...
run_tier0() {
    ...
    while IFS= read -r name; do
        ...
        # Dockerfile 检查（cloudbaserun 容器模板）
        if [ -f "$dir/Dockerfile" ]; then
            report_add "$name" "tier0:dockerfile" ok "Dockerfile exists"
        fi
        ...
        if [ "$type" = "http" ]; then
            ...
            if [ -f "$bs" ]; then
                ...
                # cloudbaserun 容器模板不需要 scf_bootstrap
                if [ "$type" = "container" ]; then
                    : # skip bootstrap check for container templates
                else
...
run_tier1() {
    ...
    while IFS= read -r name; do
        ...
        if [ "$type" = "http" ]; then
            ...
            # container / unknown type: skip local runtime checks
            report_add "$name" "tier1" skip "type=$type not supported for local run"
        fi
    done <<< "$names"
}
