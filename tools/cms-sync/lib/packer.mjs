...
// ---------- 导出：分析打包类型 ----------
export function detectPackType(templatesDir, dirName) {
    const jsonPath = join(templatesDir, dirName, 'cloudbase-template.json');
...
export function isScfWeb(templatesDir, dirName) {
    const jsonPath = join(templatesDir, dirName, 'cloudbase-template.json');
...
// ---------- 导出：运行 build.sh ----------
export function runBuild(templatesDir, dirName) {
...
// ---------- 获取实际部署文件列表 ----------
function getDeployFiles(templatesDir, dirName, packType) {
    const dirPath = join(templatesDir, dirName);
    const isWeb = isScfWeb(templatesDir, dirName);
...
// ---------- 导出：打包 ----------
export function pack(templatesDir, dirName) {
    ...
    const dirPath = join(templatesDir, dirName);
    const deployFiles = getDeployFiles(templatesDir, dirName, packType);
