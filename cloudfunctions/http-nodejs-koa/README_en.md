...
A CloudBase HTTP function template powered by the [Koa](https://koajs.com/) framework.
...
├── index.js            # Koa application source
├── package.json
...
本目录提供了 `cloudbaserc.json`（已声明 `type: http`、`path: /http-nodejs-koa`），安装并登录 [CloudBase CLI](https://docs.cloudbase.net/cli/intro) 后即可一键部署。

This folder ships a ready-to-use `cloudbaserc.json` (with `type: http`, `path: /http-nodejs-koa`). After installing & logging in to the [CloudBase CLI](https://docs.cloudbase.net/cli/intro), deploy in one command.
...
curl https://<YOUR_ENV_ID>.service.tcloudbase.com/http-nodejs-koa/

# 4. 查看日志 / Tail logs
tcb fn log http-nodejs-koa -e <YOUR_ENV_ID>
```
