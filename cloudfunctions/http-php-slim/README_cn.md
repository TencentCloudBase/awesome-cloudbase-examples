...
使用 [Slim 4](https://www.slimframework.com/) 框架的 CloudBase HTTP 函数模板。
...
1. 执行 `composer install` 在本地生成 `vendor/`。
2. 整个目录（包含 `vendor/`）上传到 CloudBase HTTP 函数。
