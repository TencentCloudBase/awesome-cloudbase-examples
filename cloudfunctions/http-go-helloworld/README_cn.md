...
使用 Go 标准库 `net/http` 实现的最小化 CloudBase HTTP 函数示例。
...
```bash
./build.sh
# 将整个目录（含编译产物 main）打包上传到云函数；运行环境选择 CustomRuntime。
```

或在 CloudBase 控制台新建「HTTP 触发函数」，运行环境选择 CustomRuntime，上传含 `scf_bootstrap` 与 `main` 的产物即可。
