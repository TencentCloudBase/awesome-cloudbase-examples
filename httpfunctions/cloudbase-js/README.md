...
cloudbase-js/
├── src/
│   ├── index.js                           # Express 应用入口，路由注册
│   ├── agent.js                           # CloudbaseAgent (AG-UI AbstractAgent)
│   ├── adapters/
│   │   └── context-adapter.js             # Express Request → CloudbaseContext 转换
│   ├── middlewares/
│   │   └── detect-user.js                 # JWT 用户认证中间件
│   ├── services/
│   │   ├── chat_main.service.js           # 主对话服务（SSE 直出 chat() + AG-UI 透传 chatStream()）
│   │   ├── chat_history.service.js        # 历史记录 CRUD（云开发数据模型 API）
│   │   ├── chat_context.service.js        # 对话上下文组装（系统提示词、历史、工具结果）
│   │   ├── chat_tool.service.js           # 工具调用（联网搜索、知识库、数据模型、文件、语音）
│   │   ├── conversation_relation.service.js # 会话关系 CRUD
│   │   └── recommend_questions.service.js # 推荐问题生成（SSE 流式）
│   ├── llm/
│   │   ├── llm_communicator.js            # LLM 通信器（streamEvents 透传 + stream SSE + text 非流式）
│   │   └── mcp.js                         # MCP 客户端（SSE/Streamable/Post 三种传输）
│   ├── config/
│   │   ├── env.js                         # 环境变量读取（envId 优先级、apiKey 等）
│   │   ├── bot_config.js                  # bot-config.yaml 解析（单例模式）
│   │   └── bot_info.js                    # BotInfo 类定义
│   └── utils/
│       ├── constants.js                   # 常量（数据源名称、角色、触发源等）
│       ├── helpers.js                     # 工具函数（randomId、safeJsonParse 等）
│       └── tcb.js                         # 云开发工具（文件信息获取、MCP 图片处理、envId/README 替换）
...
// SendMessageRequest
{
  ...
  state: {},                // 可选
  forwardedProps: {         // 可选
    msg: "string",          // 用户消息，覆盖从 messages 中提取的最后一条消息
    files: [],              // 文件列表
    searchEnable: false,    // 是否启用联网搜索
    needSSE: false,         // 是否使用 SSE 模式
    history: []             // 聊天历史，覆盖从 messages 中提取的历史
  }
}
```
...
### LLMCommunicator (`llm/llm_communicator.js`)
...
### ChatContextService (`services/chat_context.service.js`)
...
项目已包含 Dockerfile（单阶段构建）：

```dockerfile
# JavaScript 版本无需编译，直接使用单阶段构建
FROM node:20-alpine
WORKDIR /app
COPY package*.json ./
...
1. **ESM 模块**：项目使用 ESM（`"type": "module"`），所有导入需使用 `import` 语法并包含 `.js` 后缀
