...
import { BOT_ROLE_ASSISTANT, BOT_ROLE_USER, BOT_TYPE_TEXT } from "../utils/constants.js";
export class ChatContextService {
  ...
  fixHistory(historyList) {
    ...
    const isValidMessage = (msg) => msg?.content && msg.content.length > 0;
  ...
  async genSystemPromptMessage({
    ...
    const message = {
      ...
      content: `
<time>
  当前时间是: ${(/* @__PURE__ */ new Date()).toISOString()}, 
</time>
...
`;
        ...
        messages.push({
          ...
          content: handleSearchDBResult?.result?.searchResult?.answerPrompt ? `明白了。根据您提供的数据查询结果和要求，当遇到 "${msg}" 的问题时，我只能依据数据查询结果来回答，不添加任何推理内容或不在结果中的信息` : "好的"
        });
        return messages;
      }
      return [];
  ...
  async prepareMessages({
    ...
    if (!fixHistoryList?.length) {
      const dbHistory = await this.chatHistoryService.queryForLLM(
        this.botContext.info.botId,
        void 0,
        triggerSrc
      );
      fixHistoryList = dbHistory.map((item) => ({
        role: item.role,
    ...
    if (!this.botContext.info?.type || this.botContext.info.type === BOT_TYPE_TEXT) {
      if (fixHistoryList?.length > 0) {
        messages.push(...fixHistoryList.map((item) => ({
          role: item.role,
          content: item.content
        })));
      }
      let toolMessages = await this.callTools({ msg, searchEnable, files, needSSE });
      if (toolMessages.length === 0) {
        toolMessages = await this.genSystemPromptMessage({
          botInfoText: this.genBotInfoText(),
          toolPrompt: ""
        });
      }
