...
`
  ...
  async prepareMessages({
    ...
    // 整理历史信息，确保顺序正常
    let fixHistoryList: ChatHistoryItem[] = this.fixHistory(history)
    ...
    if (
      ...
      if (fixHistoryList?.length > 0) {
        // 转换 ChatHistoryItem 到 Record<string, string>
      ...
      // 当 callTools 返回空数组（如 databaseModel 搜索无结果）时，回退生成默认系统提示
      ...
      // 仅在 callTools 返回的最后一条消息不是 assistant 时才追加，避免连续 assistant 消息
