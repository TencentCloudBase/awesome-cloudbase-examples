...
Page({
  data: {
    ...
    // 商家转账信息（升级版 - 单笔模式）
    transferAmount: 30,          // 转账金额（分），默认 0.3 元
    transferRemark: '测试转账',   // 转账备注
    transferSceneId: '1000',     // 转账场景ID（1000=现金营销）
    outBillNo: '',               // 商户单号（用于查询）
    transferBillNo: '',          // 微信转账单号
    transferLoading: false,      // 转账加载状态
  ...
  async _call(action, data) {
    const res = await callPayCommon(action, data)
  ...
  async handleQuery() {
    ...
    try {
      ...
      if (res.code === 0 && res.data) {
        const tradeState = res.data.trade_state
        const stateDesc = getTradeStateDesc(tradeState)
        console.log('tradeState:', tradeState, stateDesc)
        const transactionId = res.data.transaction_id || ''
        const totalFee = res.data.amount?.total || this.data.totalFee
        
        this.setData({
          tradeState,
          transactionId,
          totalFee,
          refundFee: totalFee,  // 默认全额退款
        })
    ...
    try {
      ...
      if (queryResult.code === 0 && queryResult.data) {
        ...
        if (tradeState === 'SUCCESS') {
          ...
          wx.showModal({
            title: '订单已支付',
            content: `该订单已完成支付（¥${((queryResult.data.amount?.total || 0) / 100).toFixed(2)}）\n\n无法关闭订单，是否需要退款？`,
            confirmText: '去退款',
            cancelText: '取消',
            success: (res) => {
              if (res.confirm) {
  ...
  async showRefundDialog() {
    ...
    try {
      ...
      if (queryResult.code === 0 && queryResult.data) {
        const tradeState = queryResult.data.trade_state
        const totalFee = queryResult.data.amount?.total || 0
        const transactionId = queryResult.data.transaction_id || ''

        // 检查订单状态
        if (tradeState !== 'SUCCESS') {
          wx.showModal({
            title: '无法退款',
            content: `订单状态为"${queryResult.data.trade_state_desc}"，只有已支付的订单才能退款`,
            showCancel: false,
          })
          return
        }
  ...
  async handleRefund() {
    ...
    try {
      ...
      const refundResult = await this._call('wxpay_refund', {
        ...
        amount: {
          total: this.data.totalFee,      // 订单总金额
          refund: this.data.refundFee,    // 退款金额
          currency: 'CNY',
        },
      })
  ...
  async handleQueryRefund() {
    ...
    try {
      ...
      if (queryResult.code === 0) {
        const refundStatus = queryResult.data?.status
  ...
  async handleTransfer() {
    ...
    try {
      ...
      if (res.code === 0 && res.data) {
        ...
        const mchId = res.data.mchId || '' // 后端透传的商户号
